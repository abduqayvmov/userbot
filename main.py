...
from telethon.tl.functions.payments import getSavedStarGifts
from telethon.tl.functions.payments import toggleSavedStarGiftInProfile
...
async def hide_all_gifts(event):
    ...
    try:
        ...
        # 🔄 Telegram'dagi BARCHA hadyalarni bo'lib-bo'lib (paketlab) to'liq yuklab olamiz
        while True:
            result = await client(getSavedStarGifts(
                peer='me',
                offset=offset,
                limit=100  # Har safar 100 tadan yuklaydi
            ))
            ...
            # Agar keyingi sahifa bo'lsa, offset'ni yangilaymiz, bo'lmasa to'xtaydi
        ...
        for saved_gift in all_gifts:
            if getattr(saved_gift, 'unsaved', False) == False:
                await client(toggleSavedStarGiftInProfile(
                    peer='me',
                    saved_gift_id=saved_gift.id,
                    saved=False
                ))
                hidden_count += 1
...
async def show_all_gifts(event):
    ...
    try:
        ...
        # Barcha yashirilgan hadyalarni ham paketlab to'liq yuklaymiz
        while True:
            result = await client(getSavedStarGifts(
                peer='me',
                offset=offset,
        ...
        for saved_gift in all_gifts:
            if getattr(saved_gift, 'unsaved', False) == True:
                await client(toggleSavedStarGiftInProfile(
                    peer='me',
                    saved_gift_id=saved_gift.id,
                    saved=True
